...
//		손님이 몇번 오답을 입력할지 모르니 while문 사용
		while (true) {
            System.out.println(message);
            order = sc.next();
//          boolean타입 stock 초기화
            boolean stock = false;
//          boolean타입으로 i번째에 위치한 shoes에 값과 입력한 order가 같을 때
//          stock에 true를 저장하고 for문 break
//      	for문은 손님이 입력한 값과 shoes에 담긴 품목이 일치한지 검사하는 역할
            for (int i =0; i< shoes.length; i++) {
//            	손님이 입력한 order와 shoes의 i번째 값이 일치하면
                if (shoes[i].equals(order)) {
//                	stock에 true값을 저장하고 for문 여기서 종료
                	stock = true;
                    break;
                }
            }
            if (stock) {//만약 stock값이 true라면 while문 종료
                break;
